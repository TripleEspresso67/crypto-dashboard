...
function resample1dToNDays(dailyCandles, days) {
  const block = Math.max(1, Math.floor(days));
  ...
  for (const candle of dailyCandles) {
    ...
    const groupKey = year * 1000 + Math.floor(dayOfYear / block);
 ...
 * TradingView resets the cycle on January 1st of each year, so the last
 * bar of each year may contain only 1-2 days.
 */
export function resample1dTo3d(dailyCandles) {
  return resample1dToNDays(dailyCandles, 3);
}
...
export async function fetchAllCandles(symbol, interval, startTime) {
  if (interval === '2d' || interval === '3d') {
    ...
    const days = interval === '2d' ? 2 : 3;
    return resample1dToNDays(daily, days);
  }
...
export function getWarmupStart() {
  return new Date('2017-01-01T00:00:00Z').getTime();
}

export const ASSET_CONFIGS = [
  { symbol: 'BTCUSDT', name: 'BTC', interval: '2d', strategy: 'LTTI', label: 'BTC (LTTI 2D)' },
